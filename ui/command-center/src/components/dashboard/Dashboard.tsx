      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 24, marginBottom: 24 }}>
        ...
        <div style={{
          ...
          <div style={{ flex: '0 0 auto' }}>
            <Mobius size={260} state={mobiusState} glow={1} />
          </div>
        </div>
...
function HomeChat({ agentName }: { agentName: string }) {
  ...
  // Connect to existing session on mount (if one exists) so messages are visible
  useEffect(() => {
  ...
  return (
    <div>
      <SectionTitle title={`Chat with ${agentName}`} />
      <div style={{
