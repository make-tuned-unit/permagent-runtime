          ...
          return (
            <button key={p.id} onClick={() => setSelected(p.id)}
              style={{
                ...
                border: active ? `1px solid ${color.cyan}` : '1px solid rgba(255,255,255,0.06)',
                boxShadow: active ? '0 0 0 3px rgba(0,213,255,0.12)' : 'none',
                textAlign: 'left', transition: `all 200ms ${ease.out}`,
              }}>
              <div style={{ fontFamily: font.body, fontSize: 14, fontWeight: 600, color: color.text, marginBottom: 4 }}>
                {p.label}
              </div>
              <div style={{ fontFamily: font.body, fontSize: 11, color: color.textMuted, lineHeight: 1.4 }}>
                {p.traits.join(' · ')}
              </div>
            </button>
          );
