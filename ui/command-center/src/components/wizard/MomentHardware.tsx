...
export function MomentHardware({ onAdvance }: Props) {
  ...
  const handleSetup = async () => {
    ...
    if (modelInstalled) {
      ...
      await applyConfig(selectedModel, true);
      setPhase('ready');
      return;
    }
  ...
  async function applyConfig(model: string, enabled: boolean) {
    try {
      ...
      {phase === 'error' && (
        <>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
            <GhostLink onClick={() => setPhase('scanning')}>Retry</GhostLink>
