...
//!
//! The daemon token is read from ~/.permagent/secrets/daemon_token.json.

use serde::{Deserialize, Serialize};

/// Read the daemon auth token from ~/.permagent/secrets/daemon_token.json.
...
/// Emit an activity event to the daemon's event bus.
///
/// Called by frontend surfaces (browser, terminal, project picker) to report
/// user activity. The daemon token is read from disk, event is POSTed to
/// http://127.0.0.1:3001/activity/emit with Bearer auth.
#[tauri::command]
pub async fn emit_activity(
    ...
    let event = EmitPayload {
        ...
        project_id,
        payload,
        tier: "ephemeral".to_string(), // Server overrides with canonical tier
    };
