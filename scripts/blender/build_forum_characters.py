...
SOURCE = ROOT/'assets/world/forum/characters'
PUBLIC = ROOT/'ui/command-center/public/world/forum-characters'
...
def silhouette(identity, body, gold, joint, trim):
    # Distinctions must read at a distance, beyond trim color or embroidery.
    if identity == 'henry':
        plate('Diplomat long mantle',(0,1.07,-.30),(.72,1.10,.09),'spine',body)
        for sign in [-1,1]: plate('Mantle broad clasp',(sign*.35,1.62,.1),(.20,.12,.3),'spine',gold)
    elif identity == 'librarian':
        plate('Archive tower backpack',(0,1.38,-.40),(.52,.82,.23),'spine',joint)
        for i in range(6): plate('Oversize archival folio',((i-2.5)*.09,1.43,-.54),(.075,.7,.14),'spine',gold)
        for sign in [-1,1]:plate('High scholar collar',(sign*.22,1.79,-.08),(.1,.3,.3),'spine',body)
    elif identity == 'reader':
        plate('Document optical boom',(.32,2.02,.19),(.38,.06,.07),'head',joint)
        ellipsoid('Large monocular optic',(.43,2.02,.26),(.14,.14,.08),'head',gold)
        plate('Reader open folio',(.49,.91,.29),(.38,.45,.055),'foreL',body)
    elif identity == 'watcher':
        plate('Watchtower asymmetric cape',(-.24,1.18,-.27),(.44,.95,.07),'spine',body)
        plate('Observer visor hood',(0,2.10,.09),(.54,.10,.39),'head',joint)
        plate('Long range aerial',(-.26,2.28,-.16),(.045,.5,.04),'head',gold)
    elif identity == 'steward':
        plate('Broad workshop apron',(0,.93,.26),(.57,.78,.055),'spine',body)
        plate('Maintenance tool handle',(.57,1.0,.09),(.065,.8,.07),'foreL',joint)
        plate('Open wrench head',(.57,1.42,.09),(.28,.12,.09),'foreL',gold)
        for x in [.45,.69]:plate('Wrench jaw',(x,1.52,.09),(.055,.17,.09),'foreL',gold)
    elif identity == 'strix':
        for sign in [-1,1]:
            plate('Sentinel broad shoulder shield',(sign*.43,1.56,0),(.34,.35,.42),'arm'+('L' if sign==1 else 'R'),body)
        plate('Sentinel guard shield',(-.57,1.04,.08),(.14,.72,.5),'foreR',joint)
        plate('Guard split crest',(0,2.27,0),(.07,.31,.40),'head',gold)
    elif identity == 'financier':
        plate('Treasurer tall ledger',(.50,.83,.12),(.34,.49,.09),'foreL',joint)
        for i in range(3):plate('Ledger edge',( .50,.67+i*.13,.175),(.26,.035,.02),'foreL',gold)
        plate('Formal high collar',(0,1.69,-.06),(.33,.20,.21),'spine',body)
    elif identity == 'forecaster':
        for sign in [-1,1]:
            plate('Navigator long robe',(sign*.20,.63,-.16),(.28,.93,.09),'thigh'+('L' if sign==1 else 'R'),body)
        bpy.ops.mesh.primitive_torus_add(major_radius=.40,minor_radius=.045,major_segments=32,minor_segments=6,location=position((0,1.48,-.34)),rotation=(math.pi/2,0,0))
        finish(bpy.context.object,'Oversize celestial instrument','spine',gold)
        plate('Survey telescope',(.51,1.06,.09),(.12,.51,.16),'foreL',joint)
    elif identity == 'council':
        for i in [-1,0,1]:plate('Three voices crown',(i*.19,2.29,0),(.12,.24+(i==0)*.14,.14),'head',gold)
        plate('Consular shoulder bridge',(0,1.63,-.1),(.96,.12,.34),'spine',body)
        for sign in [-1,1]:plate('Consular long stole',(sign*.23,1.10,.30),(.16,.86,.05),'spine',gold)
    elif identity == 'polybot':
        plate('Industrial square visor housing',(0,1.99,0),(.48,.37,.39),'head',joint)
        plate('Industrial visor',(0,1.99,.21),(.34,.06,.025),'head',trim,'trim')
        plate('Automation dorsal power pack',(0,1.25,-.40),(.65,.67,.23),'spine',body)
        for i in range(5):plate('Cooling radiator',((i-2)*.12,1.3,-.55),(.035,.52,.14),'spine',gold)
    elif identity == 'picker':
        plate('Wide expedition brim',(0,2.13,.07),(.75,.06,.53),'head',body)
        plate('Candidate sample satchel',(-.33,.83,-.08),(.28,.42,.26),'root',joint)
        for i in range(3):plate('Ranked candidate card',(.50,.85+i*.12,.2),(.25,.075,.04),'foreL',gold)
    elif identity == 'growth_measurement':
        plate('Botanical field pack',(0,1.32,-.35),(.44,.6,.18),'spine',body)
        plate('Measurement staff',(.57,1.0,.1),(.055,1.8,.055),'foreL',gold)
        for i in range(7):plate('Staff measurement notch',(.60,.35+i*.22,.13),(.05,.02,.02),'foreL',joint)
        for i in range(3):ellipsoid('Leaf shaped sample',(-.30+i*.06,1.60+i*.08,-.08),(.08,.16,.03),'spine',gold)
...
for identity,(tone,width,gear) in PROFILES.items():
    ...
    silhouette(identity,body,gold,joint,trim)
    ...
    assert (PUBLIC/(identity+'.glb')).stat().st_size < 1_500_000
    print('CHARACTER_EXPORTED',identity)

# Render a transparent full-body portrait for every identity from its source.
for identity in PROFILES:
    bpy.ops.wm.open_mainfile(filepath=str(SOURCE/(identity+'.blend')))
    scene=bpy.context.scene
    scene.render.engine='CYCLES';scene.cycles.samples=12
    scene.render.resolution_x=360;scene.render.resolution_y=480;scene.render.resolution_percentage=100
    scene.render.film_transparent=True
    bpy.ops.object.camera_add(location=(3.0,-5.5,2.9))
    cam=bpy.context.object;cam.rotation_euler=(Vector((0,0,1.25))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='ORTHO';cam.data.ortho_scale=2.95;scene.camera=cam
    for loc,power,size in [((2,-4,6),450,4),((-3,-2,3),250,3),((1,3,4),600,3)]:
        bpy.ops.object.light_add(type='AREA',location=loc);o=bpy.context.object;o.data.energy=power;o.data.shape='DISK';o.data.size=size;o.rotation_euler=(Vector((0,0,1.2))-o.location).to_track_quat('-Z','Y').to_euler()
    scene.render.image_settings.file_format='PNG';scene.render.filepath=str(PUBLIC/(identity+'.png'))
