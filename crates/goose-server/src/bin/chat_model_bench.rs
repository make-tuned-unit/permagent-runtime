//! Chat-model bench — which model should answer a CHAT reply turn?
//!
//! A chat turn is not a voice turn. It carries the same large system prompt
//! (the daemon's capability inventory plus the tool schemas — PR #1118 put the
//! capability inventory alone at ~17k tokens) but nothing downstream of it cares
//! about spoken syllables: there is no TTS, no speakable-boundary flush, no
//! "first spoken sentence" gate. What the user actually experiences is a
//! spinner until the first token, then a stream of text they read. So this
//! bench measures time-to-first-token and a *readability* proxy — the time to
//! the first sentence-ending boundary in the streamed text — instead of the
//! voice bench's TTS-anchored measurements.
//!
...
//!   * `ttft_ms` — time to the first non-empty text delta
//!   * `first_sentence_ms` — time to the first sentence-ending boundary in the
//!     accumulated text (a LOCAL helper, not the voice path's TTS-oriented
//!     `find_speakable_boundary` — see [`first_sentence_boundary`]); this is
//!     a readability proxy, not a claim about how the daemon actually renders
//!     chat text
//!   * total stream latency, input/output/cache tokens, prompt-cache hit rate
//!     and cost from the canonical pricing table (fail-closed: an unpriced
//!     model refuses to run rather than being reported as free)
//!   * whether the expected tool was called, on the ten turns that need one
//!   * `silent_tool_call` — a tool call with NO text at all; the user's spinner
//!     spins through the whole tool round-trip with nothing to read
//!
//! Inputs (nothing personal is read or written):
//!   * `--turns`  the synthetic turn corpus, `crates/goose-server/tests/fixtures/chat_bench_turns.json`
//!   * `--system` the repo's own prompt-manager snapshot, which is the real
//!     ~100k-char stable prefix with no user data in it — this is deliberate:
//!     the chat prompt carries the full capability inventory and this bench
//!     measures it AS-IS, not a trimmed-down version
//!   * `--tools`  the tool schemas, extracted from a daemon request log by
//!     `scripts/bench/extract_bench_tools.py` (schemas only — see that script)
//!
//! Usage:
//!   cargo run --release -p permagent-daemon --bin chat_model_bench -- --probe
//!   cargo run --release -p permagent-daemon --bin chat_model_bench -- --dry-run
//!   cargo run --release -p permagent-daemon --bin chat_model_bench -- --run --out results.json
...
/// The default candidate slate: `label=provider/model`. These are the nine
/// models the chat default-selection decision cares about.
const DEFAULT_CANDIDATES: &[&str] = &[
    ...
    "dsreason=custom_deepseek/deepseek-reasoner",
...
/// Chat replies run longer than a spoken turn — there is no TTS to keep short
/// for. Still capped so a model can't win latency by truncating, and to bound
/// output spend.
const MAX_OUTPUT_TOKENS: i32 = 2048;

/// Cross-family judge, relative to most of the default slate. Documented
/// caveat: when `gpt54mini` (openai/gpt-5.4-mini) is itself a candidate, this
/// default judge is SAME-FAMILY for that one row — a model grading its own
/// family's prose is not an independent verdict. The code does not special-case
/// this; it only warns at startup (see `main`) when the judge's provider
/// matches a candidate's provider, so read that row's quality score with
/// caution rather than trusting it blindly.
const DEFAULT_JUDGE: &str = "openai/gpt-5.4-mini";
...
struct TurnResult {
    turn_id: String,
    kind: String,
    /// What a good CHAT answer must do — handed to the judge verbatim.
    rubric: String,
    /// Milliseconds to the first non-empty text delta.
    ttft_ms: Option<u128>,
    /// Milliseconds to the first sentence-ending boundary in the accumulated
    /// text — a readability proxy, not a claim about the daemon's own render
    /// path. See [`first_sentence_boundary`].
    ...
    /// A tool call with NO text at all. The user's spinner spins through the
    /// whole tool round-trip with nothing to read — it matters for chat the
    /// same way it matters for voice, just for a different reason (no audio
    /// vs. no visible progress).
    silent_tool_call: bool,
    /// The chat text, kept so a judge can score it. Synthetic prompts only.
    reply_text: String,
    /// Whether the model emitted a thinking block before replying.
    thought: bool,
    error: Option<String>,
}
...
struct Args {
    ...
    /// `provider/model` of the quality judge. Ideally a DIFFERENT family than
    /// the candidates it scores — a model grading its own family's prose is not
    /// an independent verdict. Defaults to [`DEFAULT_JUDGE`]; pass an empty
    /// string to disable judging entirely.
    judge: Option<String>,
}
...
fn parse_args() -> Result<Args> {
    let mut args = Args {
        ...
        runs: 2,
        limit: None,
        turns_path: "crates/goose-server/tests/fixtures/chat_bench_turns.json".to_string(),
        system_path:
            "crates/goose/src/agents/snapshots/permagent__agents__prompt_manager__tests__all_platform_extensions.snap"
                .to_string(),
        tools_path: "/tmp/chat_bench_tools.json".to_string(),
        out: None,
        budget_usd: DEFAULT_BUDGET_USD,
        judge: Some(DEFAULT_JUDGE.to_string()),
        judge_only: None,
    };
    ...
    while i < argv.len() {
        ...
        match argv[i].as_str() {
            ...
            "--judge" => {
                let v = next(&mut i)?;
                args.judge = if v.is_empty() { None } else { Some(v) };
            }
    ...
    let raw = std::fs::read_to_string(path).with_context(|| {
        format!(
            "reading {path} — generate it first with \
             `python3 scripts/bench/extract_bench_tools.py <daemon llm_request jsonl> {path}`"
        )
    })?;
...
/// True once `text` contains a `.`/`?`/`!` at least 12 chars in, followed by
/// whitespace or end of string.
///
/// This is a LOCAL helper, deliberately not the voice path's
/// `find_speakable_boundary` — that function exists to decide when TTS can
/// start speaking, and chat has no TTS. This one is only a readability proxy:
/// roughly, "the reader now has a complete first sentence in front of them."
/// The 12-char floor exists so a numbered-list marker like "1. " does not
/// register as a sentence boundary on its own.
fn first_sentence_boundary(text: &str) -> bool {
...
/// Stream one turn and time it the way the chat path experiences it.
async fn run_turn(
                ...
                for content in &message.content {
                    match content {
                        ...
                        MessageContent::Text(t) if !t.text.is_empty() => {
                            ...
                            if result.first_sentence_ms.is_none()
                                && first_sentence_boundary(&result.reply_text)
                            {
                                result.first_sentence_ms = Some(started.elapsed().as_millis());
                            }
                        }
    ...
    // A short reply that never crosses the 12-char floor is still readable in
    // one glance — the whole thing arrives before any boundary registers.
    // Record end-of-stream as its "sentence" time so a terse one-word answer
    // is not scored as "never had a first sentence".
    if result.first_sentence_ms.is_none() && !result.reply_text.trim().is_empty() {
        result.first_sentence_ms = Some(result.total_ms);
    }

    result.silent_tool_call =
        !result.called_tools.is_empty() && result.reply_text.trim().is_empty();
    ...
    if let Some(usage) = usage_seen {
        ...
        result.cost_usd = match maybe_get_canonical_model(provider_name, &model_config.model_name) {
            Some(canonical) => cost_of(&usage, &canonical.cost),
            None => None,
        };
    }
...
///
/// It models the shape this bench actually has: every turn sends the SAME
/// large prefix (system prompt + tool schemas), so the first turn of a run
/// pays the cache-write premium and the rest are cache reads. A provider with
/// no separate cache-read rate is charged full fresh input on every turn — for
/// those there is no credit to assume. Output is charged at the cap on every
/// turn.
///
...
/// True if `error` looks like a rate-limit response, so the summary can call
/// those out separately from other failures.
...
async fn main() -> Result<()> {
    ...
    // ~3.6 chars/token is the measured ratio for this prompt shape. Used only
    // for the budget gate, never for reporting.
    ...
    if let Some(judge_spec) = &args.judge {
        if let Some((judge_provider, _)) = judge_spec.split_once('/') {
            for (label, provider, _) in &candidates {
                if provider == judge_provider {
                    eprintln!(
                        "WARN: judge provider '{judge_provider}' matches candidate '{label}'s \
                         provider — that row's quality score is a same-family grade, not an \
                         independent verdict. Treat it with caution."
                    );
                }
            }
        }
    ...
    // `--dry-run` is also the default whenever `--run` is absent; the explicit
    // flag exists so a caller can force a dry run even alongside other flags,
    // and so the field is not merely parsed-and-discarded.
    ...
    for (label, provider_name, model_name) in &candidates {
        ...
        for run_index in 0..args.runs {
            ...
            let session_id = format!("cbench-{label}-{run_index}");
            ...
            for turn in &turns {
                ...
                println!(
                    "  {label} run{run_index} {:<4} ttft={:>6} sentence={:>6} total={:>6} tools={:?} silent={} cache_hit={:?} ${:.4} {}",
                    result.turn_id,
                    result
                        .ttft_ms
                        .map(|v| v.to_string())
...
/// The judge's mandate. It scores CHAT answers, so the rubric it applies is
/// about being *read*, not heard: direct, no padding, a short list only where
/// it earns its place. Answers arrive anonymised and shuffled, so the judge
/// cannot favour a family it recognises, and it is told to score each one on
/// its own merits rather than rank them.
const JUDGE_SYSTEM: &str = "\
You grade replies from a text CHAT assistant. Each reply is going straight to a \
chat window and will be read, not heard.

Score each candidate answer 0-5 against the stated requirement:
  5 — does exactly what the requirement asks, concisely and directly
  4 — right substance, slightly long or slightly padded
  3 — usable but padded, or hedges instead of answering
  2 — partially wrong, or buries the answer under unnecessary preamble
  1 — mostly wrong, or unusable as an answer
  0 — empty, refuses without cause, or answers a different question

Penalise: unnecessary preamble, restating the question before answering, \
meta-commentary about being an AI, and padding well past what the requirement \
needs. A short list or a code block is fine when the content actually calls for \
one — do not penalise structure that earns its place.
...
/// better than grading answers in isolation, and it costs a handful of small calls
/// instead of one huge one. Only the FIRST run of each candidate is judged — the
/// warm-cache repeat measures latency, not a different answer, and paying to
/// grade it twice buys nothing.
async fn judge_replies(
    ...
    for turn in turns {
        ...
        for (idx, run) in ordered.iter().enumerate() {
            ...
            let text = if result.reply_text.trim().is_empty() {
                "(no text — the model only called a tool)"
            } else {
...
fn summarize(runs: &[RunResult], quality: &BTreeMap<String, Quality>) {
    ...
    println!(
        "\n| candidate | run | TTFT med | TTFT p90 | first-sentence med | total med | tool ok | silent | cache hit | $/turn | thinks | quality |"
    );
    println!("|---|---|---|---|---|---|---|---|---|---|---|---|");
    for (name, (candidate, turns)) in by_candidate {
        let mut ttft: Vec<u128> = turns.iter().filter_map(|t| t.ttft_ms).collect();
        let mut sentence: Vec<u128> = turns.iter().filter_map(|t| t.first_sentence_ms).collect();
        ...
        let silent = turns.iter().filter(|t| t.silent_tool_call).count();
        ...
        // than quietly showing a median over the handful that survived. Rate
        // limits are called out separately since they are a bench artifact, not
        // a quality signal about the model.
        let failed_errors: Vec<&str> = turns.iter().filter_map(|t| t.error.as_deref()).collect();
        let failed = failed_errors.len();
        let rate_limited = failed_errors
            .iter()
            .copied()
            .filter(|e| is_rate_limit_error(e))
            .count();
        let name = if failed > 0 {
            let rl_note = if rate_limited > 0 {
                format!(", {rate_limited} rate-limited")
            } else {
                String::new()
            };
            format!("{name} — {failed} FAILED{rl_note}")
        } else {
            name
        };
        ...
        println!(
            "| {name} | {} | {} | {} | {} | {}/{} | {silent} | {mean_hit} | {mean_cost} | {}/{} | {quality_cell} |",
            percentile(&ttft, 0.5)
                .map(|v| v.to_string())
                .unwrap_or_else(|| "-".into()),
            percentile(&ttft, 0.9)
...
mod tests {
    ...
    #[test]
    fn parse_candidate_accepts_label_provider_model() {
        let (label, provider, model) = parse_candidate("haiku=anthropic/claude-haiku-4-5-20251001")
            .expect("well-formed candidate spec should parse");
    ...
    #[test]
    fn parse_candidate_rejects_missing_equals() {
        assert!(parse_candidate("anthropic/claude-haiku-4-5-20251001").is_err());
    }

    #[test]
    fn parse_candidate_rejects_missing_slash() {
        assert!(parse_candidate("haiku=anthropic-claude-haiku").is_err());
    }

    #[test]
    fn parse_candidate_rejects_empty_spec() {
    ...
    #[test]
    fn percentile_p90_of_ten_values() {
        let v: Vec<u128> = (1..=10).collect();
        // idx = round(9 * 0.9) = round(8.1) = 8 -> v[8] == 9
        assert_eq!(percentile(&v, 0.9), Some(9));
    }

    #[test]
    fn sentence_boundary_requires_at_least_twelve_chars() {
        // Too short: the period lands before the 12-char floor.
        assert!(!first_sentence_boundary("Hi there."));
    }

    #[test]
    fn sentence_boundary_found_after_floor() {
        assert!(first_sentence_boundary(
            "This is a longer sentence. And more follows."
        ));
    }

    #[test]
    fn sentence_boundary_accepts_question_and_exclamation() {
        assert!(first_sentence_boundary("Is this a long enough question?"));
        assert!(first_sentence_boundary("This is long enough to count!"));
    }

    #[test]
    fn sentence_boundary_requires_trailing_whitespace_or_end() {
        // "12.5" — the period is followed by a digit, not whitespace or end.
        assert!(!first_sentence_boundary(
            "The reading was exactly 12.5 units"
        ));
    }

    #[test]
    fn sentence_boundary_none_when_absent() {
        assert!(!first_sentence_boundary(
            "no terminal punctuation in this text at all"
        ));
    }

    #[test]
    fn extract_json_object_finds_embedded_object() {
        let text = "here is the answer: {\"A\": 4, \"B\": 2} thanks";
        let parsed = extract_json_object(text).expect("should find the object");
    ...
    #[test]
    fn is_rate_limit_error_matches_common_phrasing() {
    ...
    #[test]
    fn projected_cost_refuses_unpriced_model() {
        // No canonical row exists for this made-up provider/model pair, so the
        // pre-flight projection must return None (fail closed) rather than a
        // silent zero — the caller then refuses to run rather than assume free.
        let projected = projected_cost("not-a-real-provider", "not-a-real-model", 70_000, 30, 2);
