...
use permagent::agents::{Container, ExtensionLoadResult};
use permagent::goose_apps::{fetch_mcp_apps, GooseApp, McpAppCache};

use base64::Engine;
use permagent::agents::reply_parts::is_tool_visible_to_app;
...
use permagent::recipe_deeplink;
use permagent::session::session_manager::SessionType;
use permagent::session::{EnabledExtensionsState, ExtensionState, Session};
use permagent::{
...
async fn start_agent(
    ...
    permagent::posthog::set_session_context("desktop", false);
    ...
    let original_recipe = if let Some(deeplink) = recipe_deeplink {
        match recipe_deeplink::decode(&deeplink) {
            ...
            Err(err) => {
                ...
                #[cfg(feature = "telemetry")]
                permagent::posthog::emit_error("recipe_deeplink_decode_failed", &err.to_string());
    ...
    let mut session = manager
        ...
        .map_err(|err| {
            ...
            #[cfg(feature = "telemetry")]
            permagent::posthog::emit_error("session_create_failed", &err.to_string());
            ErrorResponse {
...
async fn resume_agent(
    ...
    #[cfg(feature = "telemetry")]
    permagent::posthog::set_session_context("desktop", true);
    ...
    let session = state
        ...
        .map_err(|err| {
            ...
            #[cfg(feature = "telemetry")]
            permagent::posthog::emit_error("session_resume_failed", &err.to_string());
            ErrorResponse {
...
async fn call_tool(
    ...
    let ctx = permagent::agents::ToolCallContext::new(payload.session_id.clone(), None, None);
