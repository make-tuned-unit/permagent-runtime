...
use permagent::download_manager::{get_download_manager, DownloadProgress};
use permagent::providers::local_inference::hf_models::{self, HfModelInfo, HfQuantVariant};
use permagent::providers::local_inference::{
    ...
    for (model_id, url, path) in mmproj_downloads_needed {
        if !path.exists() && started_paths.insert(path.clone()) {
            ...
            let dominated_by_active = dm
                ...
                .is_some_and(|p| p.status == permagent::download_manager::DownloadStatus::Downloading);
...
pub fn routes(state: Arc<AppState>) -> Router {
    let registered_paths: std::collections::HashSet<std::path::PathBuf> = get_registry()
        ...
        .unwrap_or_default();
    permagent::download_manager::cleanup_partial_downloads(
