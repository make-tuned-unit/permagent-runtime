#!/usr/bin/env bash
# The oracle for task-05. permagent-eval copies this directory OVER the finished
...
cp ".bench_oracle/hidden_check.ts" "ts/hidden_check_task_05.ts"
run_with_timeout 120 node --experimental-strip-types "ts/hidden_check_task_05.ts"
STATUS=$?
if [ $STATUS -eq 0 ]; then echo "PASS task-05"; else echo "FAIL task-05"; fi
exit $STATUS
