#!/usr/bin/env bash
# The oracle for task-08. permagent-eval copies this directory OVER the finished
...
cp ".bench_oracle/hidden_task_08.rs" "rs/tests/hidden_task_08.rs"
run_with_timeout 180 cargo test --offline --manifest-path "rs/Cargo.toml" --test hidden_task_08
STATUS=$?
if [ $STATUS -eq 0 ]; then echo "PASS task-08"; else echo "FAIL task-08"; fi
exit $STATUS
