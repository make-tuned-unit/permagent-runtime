#!/usr/bin/env bash
# The oracle for task-07. permagent-eval copies this directory OVER the finished
...
cp ".bench_oracle/test_hidden_task_07.py" "py/tests/test_hidden_task_07.py"
# PYTHONPATH=. so the deliverable package resolves: running a file in
# tests/ puts tests/ on sys.path, not the package root.
( cd py && export PYTHONPATH=. && run_with_timeout 120 python3 "tests/test_hidden_task_07.py" )
STATUS=$?
if [ $STATUS -eq 0 ]; then echo "PASS task-07"; else echo "FAIL task-07"; fi
exit $STATUS
