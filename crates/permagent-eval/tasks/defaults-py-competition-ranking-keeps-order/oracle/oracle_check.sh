#!/usr/bin/env bash
# The oracle for task-10. permagent-eval copies this directory OVER the finished
...
cp ".bench_oracle/test_hidden_task_10.py" "py/tests/test_hidden_task_10.py"
# PYTHONPATH=. so the deliverable package resolves: running a file in
# tests/ puts tests/ on sys.path, not the package root.
( cd py && export PYTHONPATH=. && run_with_timeout 120 python3 "tests/test_hidden_task_10.py" )
STATUS=$?
if [ $STATUS -eq 0 ]; then echo "PASS task-10"; else echo "FAIL task-10"; fi
exit $STATUS
