#!/usr/bin/env bash
# The oracle for task-09. permagent-eval copies this directory OVER the finished
...
cp ".bench_oracle/hidden_check.ts" "ts/hidden_check_task_09.ts"
run_with_timeout 120 node --experimental-strip-types "ts/hidden_check_task_09.ts"
STATUS=$?
if [ $STATUS -eq 0 ]; then echo "PASS task-09"; else echo "FAIL task-09"; fi
exit $STATUS
