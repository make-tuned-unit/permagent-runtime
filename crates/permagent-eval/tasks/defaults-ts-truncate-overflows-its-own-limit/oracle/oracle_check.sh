#!/usr/bin/env bash
# The oracle for task-01. permagent-eval copies this directory OVER the finished
...
cp ".bench_oracle/hidden_check.ts" "ts/hidden_check_task_01.ts"
run_with_timeout 120 node --experimental-strip-types "ts/hidden_check_task_01.ts"
STATUS=$?
if [ $STATUS -eq 0 ]; then echo "PASS task-01"; else echo "FAIL task-01"; fi
exit $STATUS
