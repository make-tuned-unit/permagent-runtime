...
pub async fn send_goal_a2a(
    ...
    // RLM write-through so the next dispatch brief for the target quotes this
    // as data-not-instructions.
    //
    // TODO(prime-rlm, #1129): once the durable RLM store lands, call
    // `rlm::write_a2a_feedback(pool, to_goal, &msg)` (rlm.rs:636) instead of
    // this in-memory set + metadata-blob snapshot. That pair is the racy path
    // the durable `rlm_context` table replaces; the shape of what is written
    // ("a2a_feedback" keyed by the target's goal session) does not change.
