...
/// Returns true if a term is a common English word that should NOT become
/// an annotation entity ref. Prevents STT proper-noun dictionary pollution
/// (common words like "graph", "click", "navigate" were being stored as
/// entity terms and then fed into the pronunciation dictionary — see #254).
///
/// Aligned with the STT corrector's `COMMON_WORDS` stoplist in
/// `voice/proper_noun_corrector.rs` but focused on the Librarian's output
/// (inflected verb/noun forms the LLM tends to produce).
fn is_annotation_stopword(term: &str) -> bool {
    let lower = term.to_lowercase();

    // Reject single-character and very short terms
    if lower.len() < 3 {
        return true;
    }
    ...
    ANNOTATION_STOPWORDS.contains(lower.as_str())
}

/// Common English words that the Librarian's TERMS field frequently produces.
/// These are valid vocabulary but should not become entity refs in the
/// annotation table — they add noise to the knowledge graph and pollute
/// the STT proper-noun correction dictionary.
static ANNOTATION_STOPWORDS: std::sync::LazyLock<std::collections::HashSet<&'static str>> =
    std::sync::LazyLock::new(|| {
        [
            // ── Verbs & inflections (the Librarian prompt asks for these) ────
            "add",
            "added",
            "adding",
            "ask",
            ...
            "workspace",
            "wrapper",
            // ── Additional words identified as STT-polluting (#254) ──────────
            "graph",
            "click",
            "calls",
            "drive",
...
pub fn annotate_memory(
    ...
    // Extract "Related terms: ..." segment (filter common words to prevent
    // STT proper-noun dictionary pollution — see #254)
    if let Some(after_terms) = description.split("Related terms:").nth(1) {
        ...
        for term in terms_str.split(',') {
            let t = term.trim();
            if !t.is_empty() && !is_annotation_stopword(t) {
    ...
    // Extract "Categories: ..." segment (categories are broader, less filtering needed
    // but still reject very short or single-word generic terms)
...
mod tests {
    ...
    #[test]
    fn test_annotation_stopword_filters_common_words() {
        assert!(is_annotation_stopword("navigate"));
        assert!(is_annotation_stopword("navigated"));
        assert!(is_annotation_stopword("browsing"));
        assert!(is_annotation_stopword("click"));
        assert!(is_annotation_stopword("context"));
        assert!(is_annotation_stopword("graph"));
        assert!(is_annotation_stopword("database"));
        assert!(is_annotation_stopword("message"));
        assert!(is_annotation_stopword("system"));
    }

    #[test]
    fn test_annotation_stopword_keeps_proper_nouns() {
        assert!(!is_annotation_stopword("Permagent"));
        assert!(!is_annotation_stopword("Spectral"));
        assert!(!is_annotation_stopword("Supabase"));
        assert!(!is_annotation_stopword("Kubernetes"));
        assert!(!is_annotation_stopword("Gmail"));
    }

    #[test]
    fn test_annotation_stopword_rejects_short_and_numeric() {
        assert!(is_annotation_stopword("a"));
        assert!(is_annotation_stopword("ab"));
        assert!(is_annotation_stopword("42"));
        assert!(is_annotation_stopword("123"));
    }
}
