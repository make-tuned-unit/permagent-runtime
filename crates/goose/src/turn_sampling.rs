...
//!
//! It is NOT the default recall path, and this module exists to keep it from
//! becoming one by accident. Spectral's preregistered latency gate FAILED:
//! recall-only p95 regressed +87–100% against a +5% kill line, from the
//! synchronous delivery-write commit (p50 improved ~19%, so it is the write,
//! not retrieval). This repo already carries a latency incident caused by slow
//! pre-stream recall on the voice path; shipping `turn` at 100% would walk
//! straight back into it.
//!
//! So: a fraction of turns, off by default, set deliberately. When the deferred
//! delivery write lands upstream the rate can go to 1.0 and this module can go
//! away.
...
/// a latency-regressing path must never be opt-out.
