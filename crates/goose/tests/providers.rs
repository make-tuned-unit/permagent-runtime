...
use permagent::providers::anthropic::ANTHROPIC_DEFAULT_MODEL;
use permagent::providers::azure::AZURE_DEFAULT_MODEL;
use permagent::providers::base::Provider;
#[cfg(feature = "aws-providers")]
use permagent::providers::bedrock::BEDROCK_DEFAULT_MODEL;
use permagent::providers::claude_code::CLAUDE_CODE_DEFAULT_MODEL;
use permagent::providers::codex::CODEX_DEFAULT_MODEL;
use permagent::providers::create_with_named_model;
use permagent::providers::databricks::DATABRICKS_DEFAULT_MODEL;
use permagent::providers::errors::ProviderError;
use permagent::providers::google::GOOGLE_DEFAULT_MODEL;
use permagent::providers::litellm::LITELLM_DEFAULT_MODEL;
use permagent::providers::openai::OPEN_AI_DEFAULT_MODEL;
#[cfg(feature = "aws-providers")]
...
impl ProviderFixture {
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<permagent::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
        ...
        let ctx = permagent::agents::ToolCallContext::new(
    ...
    async fn test_image_content_support(&self) -> Result<()> {
        let image_config = match &self.image_model {
            Some(model) => {
                Some(permagent::model::ModelConfig::new(model)?.with_canonical_limits(&self.name))
            }
            None => None,
        };
        let response = self
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config = permagent::model::ModelConfig::new(alt)?.with_canonical_limits(&self.name);
