...
use anyhow::Result;
use permagent::agents::{Agent, AgentRunnerConfig, GoosePlatform};
use permagent::config::permission::PermissionManager;
use permagent::config::GooseMode;
use permagent::model::ModelConfig;
use permagent::providers::{create, testprovider::TestProvider};
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    permagent::agents::moim::SKIP.with(|f| f.set(true));
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn permagent::providers::base::Provider>,
            &session.id,
        )
        .await?;
