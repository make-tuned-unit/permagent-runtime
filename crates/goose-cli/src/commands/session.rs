...
use permagent::session::{generate_diagnostics, Session, SessionManager};
...
fn export_session_to_markdown(
    messages: Vec<permagent::conversation::message::Message>,
    session_name: &String,
) -> String {
    ...
    for message in &messages {
        ...
        if message.content.iter().any(|content| {
            matches!(
                content,
                permagent::conversation::message::MessageContent::ToolRequest(_)
            )
        }) {
