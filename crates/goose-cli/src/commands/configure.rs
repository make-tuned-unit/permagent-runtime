...
use console::style;
use permagent::agents::extension::{ToolInfo, PLATFORM_EXTENSIONS};
use permagent::agents::extension_manager::get_parameter_names;
use permagent::agents::Agent;
use permagent::agents::{extension::Envs, ExtensionConfig};
use permagent::config::declarative_providers::{
    create_custom_provider, remove_custom_provider, CreateCustomProviderParams,
};
use permagent::config::extensions::{
...
use permagent::config::signup_tetrate::TetrateAuth;
...
use permagent::posthog::{get_telemetry_choice, TELEMETRY_ENABLED_KEY};
use permagent::providers::base::ConfigKey;
use permagent::providers::chatgpt_codex::reasoning_levels_for_model;
use permagent::providers::formats::anthropic::supports_adaptive_thinking;
use permagent::providers::provider_test::test_provider_configuration;
use permagent::providers::{create, providers, retry_operation, RetryConfig};
...
fn select_model_from_list(
    models: &[String],
    provider_meta: &permagent::providers::base::ProviderMetadata,
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&permagent::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use permagent::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    ...
    let model_config = match permagent::model::ModelConfig::new(&configured_model) {
...
async fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = permagent::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        permagent::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
    ...
    if let Err(e) = permagent::providers::cleanup_provider(selected_id).await {
