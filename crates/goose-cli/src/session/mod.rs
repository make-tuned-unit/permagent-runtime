...
use console::Color;
use permagent::agents::AgentEvent;
use permagent::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use permagent::permission::Permission;
use permagent::permission::PermissionConfirmation;
...
use permagent::agents::{Agent, COMPACT_TRIGGERS, SessionConfig};
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: permagent::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(permagent::config::DEFAULT_EXTENSION_TIMEOUT),
            bundled: None,
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            permagent::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    pub async fn get_session(&self) -> Result<permagent::session::Session> {
        self.agent
            .config
            .session_manager
            .get_session(&self.session_id, false)
    ...
    fn save_recipe(
        &self,
        recipe: &permagent::recipe::Recipe,
        filepath_str: &str,
...
fn prompt_tool_confirmation(security_prompt: &Option<String>) -> Result<Permission> {
    ...
    let prompt = if let Some(security_message) = security_prompt {
        ...
        "Permagent would like to call the above tool, do you allow?".to_string()
    };
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<permagent::providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                permagent::providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
async fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("GOOSE_PLANNER_PROVIDER") {
        ...
        config
            .get_goose_provider()
            .expect("No provider configured. Run 'permagent configure' first")
    };
    ...
    let model = if let Ok(model) = config.get_param::<String>("GOOSE_PLANNER_MODEL") {
        ...
        config
            .get_goose_model()
            .expect("No model configured. Run 'permagent configure' first")
    };

    let model_config =
        ModelConfig::new_with_context_env(model, &provider, Some("GOOSE_PLANNER_CONTEXT_LIMIT"))?;
    let extensions = permagent::config::extensions::get_enabled_extensions_with_config(config);
