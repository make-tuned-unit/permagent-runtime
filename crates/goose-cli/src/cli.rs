...
use permagent::posthog::get_telemetry_choice;
use permagent::recipe::Recipe;
use permagent_mcp::mcp_server_runner::{serve, McpCommand};
use permagent_mcp::{AutoVisualiserRouter, ComputerControllerServer, MemoryServer, TutorialServer};
...
use permagent::agents::Container;
...
#[derive(Parser)]
#[command(name = "permagent", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), permagent::config::DEFAULT_EXTENSION_TIMEOUT);
    ...
    match (
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    permagent::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use permagent::providers::local_inference::hf_models;
    use permagent::providers::local_inference::local_model_registry::{
    ...
    match command {
        ...
        LocalModelsCommand::Download { spec } => {
            ...
            let local_path =
                permagent::config::paths::Paths::in_data_dir("models").join(&file.filename);
            ...
            loop {
                if let Some(progress) = manager.get_progress(&format!("{}-model", model_id)) {
                    match progress.status {
                        permagent::download_manager::DownloadStatus::Downloading => {
                            print!(
                                "\r  {:.1}% ({:.0}MB / {:.0}MB)",
                        ...
                        permagent::download_manager::DownloadStatus::Completed => {
                            println!("\nDownloaded: {}", model_id);
                            break;
                        }
                        permagent::download_manager::DownloadStatus::Failed => {
                            let err = progress.error.unwrap_or_default();
                            anyhow::bail!("Download failed: {}", err);
                        }
                        permagent::download_manager::DownloadStatus::Cancelled => {
                            println!("\nDownload cancelled.");
                            break;
                        }
                    }
...
pub async fn cli() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Some(Command::ValidateExtensions { file }) => {
            use permagent::agents::validate_extensions::validate_bundled_extensions;
